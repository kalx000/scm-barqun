                ...
                <v-card-text>
                  <v-container>
                    <v-row>
                      ...
                      <v-col cols="12" sm="6" md="4">
                        <v-text-field
                          v-model="editedItem.jumlah_barang"
                      ...
                      <v-col cols="12" sm="6" md="4">
                        ...
                        >
                          <template v-slot:activator="{ on, attrs }">
                            <v-text-field
                              v-model="editedItem.tanggal_pemesanan"
                              label="Date Amount"
...
export default {
  ...
  data: () => ({
    ...
    dialogEdit: false,
    dialogDelete: false,
    tanggal_pemesanan: new Date().toISOString().substr(0, 10),
    // statusOptions: [
    ...
    headers: [
      ...
      { text: "Out Date", value: "tanggal_pemesanan" },
      { text: "Outcoming Amount", value: "jumlah_barang" },
  ...
  methods: {
    ...
    async save() {
      try {
        ...
        if (this.formTitle === "Add Order") {
          console.log("Sending POST request...");
          console.log(this.editedItem);

          const response = await axios.post(
            "http://127.0.0.1:8081/api/order",
            this.editedItem,
            { headers }
          );
          axios
            .get("http://127.0.0.1:8081/api/order", { headers })
            .then((response) => {
              this.items = response.data.data;
              this.dialog = false;
            })
          ...
          await axios.put(
            `http://127.0.0.1:8081/api/order/${this.editedItem.id}`,
            this.editedItem,
            { headers }
          );
          axios
